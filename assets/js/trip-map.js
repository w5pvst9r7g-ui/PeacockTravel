...
window.TripMap = (function () {
  ...
  function applyLayerVisibility() {
    var day = activeDay !== 'all' ? window.TRIP.days[activeDay - 1] : null;
  ...
  function init() {
    ...
    if (!stage || !canvas || !window.TRIP || !window.L) return;
  ...
  function thumbSVG(n) {
    var day = window.TRIP.days[n - 1];
    if (!day) return '';
    var lookup = {};
    window.TRIP.pois.forEach(function (p) { lookup[p.id] = p; });
    var cs = [];
    day.stops.forEach(function (s) {
